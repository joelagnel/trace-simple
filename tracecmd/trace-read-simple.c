...
static struct tep_record *get_next_record(struct tracecmd_input *handle)
{
	struct tep_record *record;
	struct tep_handle *pevent;
	int found = 0;
...
int main (int argc, char **argv)
{
	...
	handle = read_trace_header("trace.dat");
	if (!handle)
...
//			add_functions(pevent, functions);
		...
		if (print_event) {
			tracecmd_print_events(handle, print_event); /* XXXXXX */
			return;
		}
#endif
...
#endif
		if (ret) {
			printf("tracecmd_read_headers failed\n");
			return 1;
		}
...
#endif
	read_data_info(handle, otype, global);
